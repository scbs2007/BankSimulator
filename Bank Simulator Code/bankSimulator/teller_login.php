...
if (isset($_POST['username'])) {
  ...
  $MM_redirectLoginSuccess = "teller_options.php";
  $MM_redirectLoginFailed = "teller_login.php";
  $MM_redirecttoReferrer = true;
  mysql_select_db($database_BankSimulatorServer, $BankSimulatorServer);
  
  $LoginRS__query=sprintf("SELECT username, password FROM tbl_teller_details WHERE username=%s AND password=%s",
  ...
  if ($loginFoundUser) {
	...
	//Hand written
	$query=sprintf("SELECT tell_id,username, password FROM tbl_teller_details WHERE username=%s AND password=%s",
    GetSQLValueString($loginUsername, "text"), GetSQLValueString($password, "text")); 
   
    $MyLoginRS = mysql_query($query, $BankSimulatorServer) or die(mysql_error());
	$row = mysql_fetch_assoc($MyLoginRS);
    echo $_SESSION['uid'] = $row['tell_id'];	
	$_SESSION['userType'] = 'T';        
...
<h1 style="background:url(images/teller_login.png) no-repeat;
text-indent:-9999px;
">Teller Login</h1>
